...
- **1px rules inside the app**, and **the marker follows the job, not the colour**. Corrected by the product owner on 16 September 2026, after the build session's seam count showed the first version of this rule was wrong. A surface the user acts on keeps its heavier marker, in whatever colour it already carries: that is `form` and `.resolution-form`, and `.resolution-form` is marked in ink rather than accent, which is why colour cannot be the test. A panel that only displays drops to 1px however it is marked: that is `.analysis-panel`, `.quality-hero` and `.routing-headline`. An accent border on a panel that only displays is the loudest thing on screen doing the least work. The 2px ink rule is retained on the page header only, as the tie back to the site.
