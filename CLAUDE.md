...
**The marketing site.** `MarketingSiteKeepsZeroRadius` enforces zero radius there, built 17 September 2026 as the site session's work. Nothing enforced it before, because the test named against the rule only ever read the Assay page. The documented exception is **three rules, four rendered elements, because the homepage rule dresses the email and LinkedIn badges together**: `.close .contact-badge` on `index.html` and on `forecast-risk.html`, and `.mail-badge` on `forecastability.html`. **The scan allows them by selector and never by value**, so `50%` on anything else is a finding. It counts rules because it reads rules. Ruled by the product owner on 16 September 2026, closing Q9, and the count corrected by him on 17 September 2026, closing Q3 in `docs/marketing-site-open-questions.md`. Recorded in full in `docs/assay-theme-open-questions.md`, and the scan's evidence is in `docs/evidence/radius-scan-evidence.md`.
