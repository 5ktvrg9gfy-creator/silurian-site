...
- **Site header renamed and given a subline, 16 September 2026. Merged, Production check outstanding.** Pull request 112, merge commit `a065284f240a365a7256dfeed1398c39deabeaf0`, from `main` at `294cfc6`. Both Vercel deployments reported Ready on the head commit `682f716`, the site project and the Forecast Diagnostic project, and the combined status was success on both. **Nobody has opened the Preview.** This session cannot reach `vercel.app`, because outbound network here is blocked by policy and the request fails at the proxy with a 403, and the product owner said merge rather than saying he had looked. So the merge rests on the local render and the suite, and the live site has not been seen by anyone since the change. **The Production smoke check is outstanding and needs him**, on `silurianconsulting.co.uk`: the header on the homepage and on one sub-page, and the footer still carrying the registered name. The header wordmark reads **Silurian** on all four marketing pages, and the homepage masthead carries a subline under it reading "Project & Programme Delivery". The footer is untouched on every page: it still carries the registered name, the company number and the registered office, which is what the product owner asked for and is what Companies House requires a site to show. The page titles, the meta descriptions and the privacy notice body also keep the registered name, because the brief changed the header and nothing else.
