...
**The Assay theme is the next build, and it cannot start until Q3 in `docs/assay-theme-open-questions.md` is answered.** The changeset "Assay app theme and dashboard shell" is approved and section 9a now permits it, but the changeset describes a dashboard Assay does not have: a 232px sidebar with two nav groups, three named metric cards, an actual-against-forecast chart and a five column product table. Assay is a masthead, a single file form, a portfolio form and an eight tab workspace. Under the product owner's correction of 16 September, "styling only, same screens, same words", every one of those structures is out of scope, and what survives has to be restated as rules against the screens that exist. That restatement is the planning session's work and has not been done. Do not start by guessing what the mock means here.
