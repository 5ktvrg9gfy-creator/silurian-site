...
- **Band 2.9 and both theme passes are verified in Production, 20 September 2026.** The phone check in `docs/production-smoke-test.md` was run by the product owner against `assay.silurianconsulting.co.uk`, built from `48e5b2c`, on an iPhone 17 Pro Max in Brave. **All nine checks pass.** The result is recorded in that file under "Recorded runs", with its limits beside it. One pass covers all three, because `48e5b2c` contains all three. **This closes the theme**: band 2.9, theme pass 1, theme pass 2, rulings 1 to 18 and all nineteen questions are done, merged and now verified.
...
Superseded on 20 September 2026 when the phone check came back, kept for the record: **the Production check was the only thing outstanding and could not be run from a build session.** It was run by the product owner and all nine checks passed.
