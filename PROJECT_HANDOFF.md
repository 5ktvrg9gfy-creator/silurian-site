...
- The site's first radius scan, 17 September 2026: `CLAUDE.md` section 9 has said zero radius since the design system was written down, and section 9a records that on this site nothing ever enforced it, because the test named against the rule only ever read the Assay page. `MarketingSiteKeepsZeroRadius` is that scan. **It found nothing.** Every non-zero radius on the four pages is an approved contact badge, and the story's instruction to stop and report a fourth rather than fix it was never triggered. **Three selectors, four elements**, which is the one number worth flagging: section 9a says three badges and three CSS rules carry a radius, but the homepage rule dresses both the email badge and the LinkedIn badge, so four render rounded. Raised as Q3 rather than corrected, because `CLAUDE.md` is shared. The three are allowed **by selector and never by value**: a scan permitting `50%` would permit the next accidental pill. One declaration would have been reported wrongly by a literal scan and is not: `index.html` writes `border-radius: var(--radius-md)` and declares `--radius-md: 0px`, so the scan resolves `var()` against the page's chain and leaves anything it cannot resolve to fail loudly. **The anti-vacuity proof is the part worth keeping.** Asserting the counts would only restate them, so `STYLE_BLOCK` was broken to match `<stylesheet>` instead of `<style>` and the tests were run against a scan that read nothing: `test_no_page_carries_a_non_zero_radius` **passed**, reporting a clean site having inspected zero CSS, while the two anti-vacuity tests and the token resolution pin all failed. That is the `.stage-tag` fault from the theme band reproduced on demand. Its anchors are structural, `body` in each page's own style block and `:root` in tokens.css, because a count floor would have had to sit under privacy.html at 16 rules and a threshold chosen to clear the shortest page is tuned rather than measured. It can also fail: `.legal a { border-radius: 6px }` planted on `privacy.html` fired two tests naming the page, the selector and the value, and the file reverted to sha256 `400d9845`. **Two zeroes that look redundant are load bearing**: `forecast-risk.html` sets an explicit `border-radius: 0` on `select` and `.btn`, which is why the rendered scan found no user agent radius on any form control. Do not tidy them. Four limits are in the test's docstring, the sharpest being that it reads declarations and not rendered elements; the rendered scan, 1664 element inspections across four pages at 1440, 768, 390 and 320px, was run once as evidence and is not committed, because Playwright is not a dependency of this suite and adding one is a decision. Raised as Q2. **No site file is in the diff and no rendered value moved.** The suite is 294 tests, up from 287. Evidence in `docs/evidence/radius-scan-evidence.md`, including a mistake made while building it: a filtered grep hid a failing test and six passes read as seven. No Preview and no Production check: outbound network is blocked in this environment and neither is claimed.
- The chart's token reads brought under the control, 17 September 2026: Q1 in `docs/marketing-site-open-questions.md` is closed and built the same day it was raised, on the product owner's instruction that a gap that specific is the one a future change walks into. `forecast-risk.html` names seven tokens as strings and asks `getComputedStyle` for each, because a canvas cannot use `var()`. `getPropertyValue` on a name nothing declares returns an empty string rather than an error, so the chart paints on with the previous colour, wrong and silent. `test_no_page_reads_an_undeclared_token_from_script` now holds those names against the same declared set the `var()` scan uses, built from the chain each page links. Both doors, one declared set, two messages that name which door. **The probe was chosen to isolate the new door rather than to pass easily.** Six of the seven also appear as `var()` in that page's CSS, so renaming one of those would have fired the old test and proved nothing new. `--color-chart-grid` is the only one read solely from script; renaming it in `tokens.css` failed exactly one test, the new one, and left the `var()` test green. Reverted, `tokens.css` byte identical. One limit is stated in the test's own docstring: JavaScript comments are not stripped, so a commented-out read still counts. That fails loudly rather than silently, which is the right way round, and stripping comments from JavaScript reliably is more machinery than the risk earns. No page file changed. The suite is 287 tests, up from 284. Evidence in `docs/evidence/chart-token-reads-evidence.md`. **Released and checked live.** Merged through pull request 121, merge commit `88830f9e4f24486674d897fecceb572dcdda4692`, from `main` at `5a18edbec950106b69a59e7e3ff4700424a9f841`. No Preview check ran: outbound network is blocked in this environment and none is claimed. The product owner checked Production on a laptop and a phone on 17 September 2026, including the masthead, and reported the site correct with nothing moved. The change is test and documentation only, so it had no rendered value to move, and the live check confirms that rather than the control.
- Spacing tokens moved and the undeclared var() control built, 17 September 2026: `--space-1` through `--space-4` were declared in `index.html` and nowhere else, so any other page naming `var(--space-4)` got nothing. An undefined custom property makes the whole declaration invalid at computed-value time: no console error, no fallback, and the page renders as though the line was never written. It had not bitten, which is luck rather than a control. Part 1 moved the four into `tokens.css` with the values unchanged. `index.html` is unchanged to the pixel, proved the way band S1 proved it: sixteen full-page screenshot hashes across four pages at four widths, all sixteen byte identical. Part 2 is the half that matters. `MarketingSiteVariablesResolve` builds each page's declared set from the chain the page itself links, read out of its own `<link>` tags rather than assumed, and fails on any `var()` naming something nothing in that chain declares. Comments are stripped first, so prose about a token is not mistaken for declaring one. Proved twice before trusted. An invented `var(--swatch-size)` on `forecastability.html` fired and was reverted. Then the historical defect itself was rebuilt, the four tokens back in `index.html` only and `privacy.html` given `var(--space-3)`, and the control named the page, the token and the chain it searched. **A first attempt at that second probe was wrong and is recorded in the evidence rather than dropped:** it fired on `index.html` instead and never reached `privacy.html`, and the grep for the expected message found nothing, which could have been read as quiet success. Three limits are stated in the test's own docstring: it is static, so a media-gated declaration counts as declared; it proves a name is declared, not that its value is sensible; and it reads `var()` and not JavaScript. **The class is half closed, not closed.** `forecast-risk.html` reads seven token names as strings through `getComputedStyle` to paint its canvas, which a canvas cannot avoid, and the new control does not see them. Raised as Q1 in `docs/marketing-site-open-questions.md` with a recommendation and not folded in, because the story asked for `var()`. The suite is 284 tests, up from 278. Evidence in `docs/evidence/spacing-tokens-evidence.md`. **Released and checked live.** Merged through pull request 120, merge commit `5a18edbec950106b69a59e7e3ff4700424a9f841`. No Preview check ran: outbound network is blocked in this environment and none is claimed. The product owner checked Production on a laptop and a phone on 17 September 2026, including the masthead, and reported the site correct with nothing moved. That is the live confirmation the sixteen byte-identical screenshot hashes predicted for the token move, taken on real browsers rather than headless Chromium.
- **Masthead label baseline-aligned and the divider drawn as a rule, 16 September 2026. Merged and checked live.** Pull request 117, merge commit `b093b9710ca94f8de7ec83a499ea328f874f91de`, from `main` at `9dfa7fd`. Built from the design session's change "masthead product label", and the full reply to it is `docs/build-response-8-2026-09-16.md`. The lockup is a grid rather than a row of flex items, so the wordmark and the nav label sit in the same grid row and **the browser does the baseline alignment instead of an offset being typed in by hand**. The pipe glyph added earlier the same evening is now a 1px rule spanning the lockup, `var(--space-4)` either side. The label takes `--color-text-muted`, 6.1:1 against the header ground where the wordmark is 10.81:1, and keeps 15px/600 sentence case, so no copy moved. Below 522px the label drops to its own row and the rule goes with it. **Production check closed 17 September 2026**, by the product owner on a laptop and a phone: the rule reads correctly and nothing else moved. This was the outstanding check named above, and it was supplied alongside the checks for pull requests 120 and 121 rather than separately.

  **The change's preferred route was refused and the product owner was told why.** It deletes "Project & Programme Delivery" from the header, which he had asked for and ruled on the same day. Two further objections are in the response: the label is the site's only navigation rather than a product name, so stacking it under the wordmark makes a link read as a wordmark's second line, and the change is addressed to "the site masthead / app header lockup", which are two products deploying from separate Vercel projects.

  **Two tokens the change names do not exist here, and this is the second time that has happened.** `--color-neutral-700` and `--color-neutral-300` came from `styles.css`, deleted on 4 September 2026 in `f2f2ca6`, exactly as recorded in `docs/build-response-6-2026-09-07.md`. Substituted with `--color-text-muted` and the site's own 1px hairline `var(--color-divider)`, and reported rather than done quietly. **The wordmark was left alone**: the change asks for 19px/-0.01em while its own "not to change" list says the wordmark does not move, and a hand-written 19px would fail `test_no_page_carries_a_hand_written_size`.

  **All four questions to the design session were answered the same day, and the answers are in the response file.** Both substitutions accepted with no new token, the hairline staying at full ink because this system does not fade its rules and the subordination is carried by the 1px weight against the header's 2px close. The wordmark does not move, and the 19px is confirmed as an error carried over from the Assay dashboard's scale. Site masthead only, Assay untouched. Sentence case at 15px confirmed. **Two things are not closed by that.** If the rule reads heavy in place the design session will look at the divider's height rather than its value, and nobody has seen it in place. And the design session's amendment writing the deleted ramp constraint into `design-decisions.md` **is not in this repository**: `docs/designdecisions.md` on `main` mentions neither the ramp, nor `styles.css`, nor `f2f2ca6`, checked by reading the file. The amended copy needs routing in the way the last version was, or the control lives only where no build session reads.

  **Evidence.** Every width from 320 to 1440, 1121 in total, on four checks each: baseline delta, the rule spanning the lockup, the rule never showing on a stacked bar, and horizontal overflow. Zero violations. The suite is 278 tests and passes. **The baseline probe was checked against a control before its numbers were believed**, and the first version failed it: two spans on one text line, which share a baseline by definition, read 0.91px apart, so that probe was measuring its own bias rather than the page. The normalised probe reads 0.00 on the control and 0.00 on the page. This is the same family as the `.stage-tag` check that returned zero observations and read as a pass. **Not seen on Preview, Production or a device**, by anyone, and this session cannot reach either.

- **A date in this file was wrong and was corrected on 16 September 2026, and the correction stays on the record.** Seven dates here and two in `docs/production-smoke-test.md` read 20 September 2026, including the header line and the Production phone check on band 2.9 and the two theme passes. Every commit in this repository is dated 16 September 2026, the phone check's own commit among them, so the two could not both be true. **The product owner ruled the 20th wrong**, and all nine now read 16 September 2026. **What is corrected is the date and nothing else**: the check was run, it passed, and the device, the browser, the address and the commit it was built from are untouched. Recorded rather than tidied away, for the reason section 13 gives: a correction nobody can see is indistinguishable from the record having always said this.

- **Site header renamed and given a subline, 16 September 2026. Merged and confirmed in Production by the product owner.** Pull request 112, merge commit `a065284f240a365a7256dfeed1398c39deabeaf0`, from `main` at `294cfc6`. Both Vercel deployments reported Ready on the head commit `682f716`, the site project and the Forecast Diagnostic project, and the combined status was success on both. **Nobody has opened the Preview.** This session cannot reach `vercel.app`, because outbound network here is blocked by policy and the request fails at the proxy with a 403, and the product owner said merge rather than saying he had looked. So the merge rests on the local render and the suite, and the live site has not been seen by anyone since the change. **The product owner confirmed the live site after the deploy landed.** He was asked for three things on `silurianconsulting.co.uk`: the header reading Silurian with the subline under it, the pipe present at desktop width and gone on a phone, and the footer still carrying the registered name and the company number. **He replied "all ok", on an iPhone 17 Max and on a PC in Microsoft Edge.** Both devices are recorded because he named them when asked. What is still not recorded, because he did not state it: which browser on the phone, which widths on the PC, and which of the three items he looked at on which device. This session cannot reach the live site to check any of it, outbound network here being blocked by policy so that `vercel.app` and the site both fail at the proxy with a 403. The confirmation is his.

  **What the two devices between them do and do not cover.** The phone covers the wrapped masthead and the hidden pipe on a real screen, which nothing else had: every browser on iOS runs WebKit, so the result holds for iOS generally whatever he opened it in. The PC covers the single-line bar with the pipe on it. **Edge is Chromium, the same engine this session measured in**, so the desktop half is a second look rather than a second engine, and no desktop Safari or Firefox has seen the page. Neither device covers Android, and no width between them is recorded, so the 520px point where the pipe goes is measured but not observed at the boundary on a device. The header wordmark reads **Silurian** on all four marketing pages, and the homepage masthead carries a subline under it reading "Project & Programme Delivery". The footer is untouched on every page: it still carries the registered name, the company number and the registered office, which is what the product owner asked for and is what Companies House requires a site to show. The page titles, the meta descriptions and the privacy notice body also keep the registered name, because the brief changed the header and nothing else.

  **Three pages got the rename and only the homepage got the subline.** The rename covers `index.html`, `forecast-risk.html`, `forecastability.html` and `privacy.html`, because a header that reads one name on the homepage and another on the next page is not a rename. The subline sits on the homepage masthead alone, where the brand is a lockup with the mark beside it. On the other three the brand is a back link sitting next to "Back to Silurian", so a tagline inside it would be clutter rather than identity. That is a judgement and it is recorded as the one open question in `docs/header-rename-open-questions.md`.

  **New markup and new CSS on the homepage, no new token.** The inline wordmark `<p>` became a `.mast-lockup` column holding `.wordmark` and `.mast-subline`, and the styles moved out of the inline attribute into the header `<style>` block beside `.mast-brand`. The wordmark keeps `--wordmark-size`, its own step outside the scale. The subline takes `--text-label`, `--color-text-muted`, 0.08em tracking and uppercase, which is the site's existing small caps line as set on `h6` and `.stat-label`. Nothing is hand written, so `test_no_page_carries_a_raw_colour_value` and `test_no_page_carries_a_hand_written_size` both still pass on the changed page rather than being stepped around.

  **The subline does not carry `white-space: nowrap` and the wordmark does.** At 320px the line is 240px wide and the masthead has 60px of gutter on the left, so pinned to one line it would push the bar past the screen. It wraps to two lines instead. Measured, not assumed.

  **A pipe separator was added between the brand block and the nav link, on the product owner's instruction after he saw the first render.** It is a `|` in a `.mast-sep` span, hidden from screen readers because it is decoration and would otherwise be read out as "vertical line". The 18px `margin-left` on the nav link and the media query that zeroed it were both removed rather than left as rules doing nothing: the air either side of the pipe is now the bar's own 14px gap, equal on both sides.

  **The pipe hides below 520px, and that number is measured rather than chosen.** With the pipe in the bar the nav sits on the brand's line down to 520px and wraps below it, so `@media (max-width: 519px)` puts the pipe exactly where there are two things on one line for it to separate, and nowhere else. The first attempt used the 620px breakpoint that was already in the file, which left a band from 505 to 619px where the bar was one line with no separator on it. The value depends on the rendered width of the two strings either side, so it is remeasured if either changes. **Verified by stepping every width from 320 to 1440px, 1121 in total: the pipe is never shown on a wrapped bar and no width overflows.**

  **Checked in a browser at 1440, 768, 390 and 320px**, with Chromium through Playwright against a local server. Horizontal overflow is 0px at every width on all four pages, no request fails, Archivo is the computed family on the subline, and the computed values are 18px/15px on the wordmark from the token and 12px `rgb(102, 97, 95)` on the subline, which is `--color-text-muted`. The masthead grows from 44px to 66px at 1440 and to 114px at 320, where the subline takes two lines. **What this does not cover:** no real device, no Preview, no Production, and nobody has looked at it who is not this session. The suite is 278 tests and passes, unchanged in count, which is expected: nothing here is pinned by a test beyond the colour and type scans.

- **Band 2.9 and both theme passes are verified in Production, 16 September 2026.** The phone check in `docs/production-smoke-test.md` was run by the product owner against `assay.silurianconsulting.co.uk`, built from `48e5b2c`, on an iPhone 17 Pro Max in Brave. **All nine checks pass.** The result is recorded in that file under "Recorded runs", with its limits beside it. One pass covers all three, because `48e5b2c` contains all three. **This closes the theme**: band 2.9, theme pass 1, theme pass 2, rulings 1 to 18 and all nineteen questions are done, merged and now verified.

  **Check 1 is the result that could not be got any other way.** It is the iOS control colour defect: a control that was never told what colour its writing should be takes the platform default, and on iOS that default is bright blue. The marketing site hit it on 6 September and `PROJECT_HANDOFF.md` recorded it as "far worse in Assay". Band 2.9 fixed it on 16 September, and **it had never been looked at on a device until this run**. Brave on iOS uses WebKit, the same engine as Safari, so the result holds for iOS generally rather than only for Brave.

  **What the pass does not cover, recorded beside it so it is not read as more than it is.** The validation bar can be amber, red or green; the app's own sample file produces amber, so **red and green are still unchecked on a phone**. A third state marker, the quality exception bar, appears only on a file with a serious problem and the app offers no such file to download, so it was not reached at all. The check covers how the app looks, not what it says or works out. And one phone is one phone: a pass on an iPhone says nothing about Android.

- **Assay theme pass 2 is built and merged, 16 September 2026.** Pull request 105, merge commit `4eb38a7a359cf7fc0aac2183bf6f3aa01fb20550`, from `main` at `cfb6ffc`. The suite is 278 tests, up from 273, and passes. Pass 2 is the adding half: 4px and one elevation step on thirteen cards, the seams under rulings 3, 7 and 8, `--slate-tint` at `#d4d6d9` on the four grid headers with the sort hover at `#c8cacd`, and the type scale at 36px/800 figures and 12px/700 labels. Rendered text on eleven screens is identical to the unmodified app. **The seam rule now has three categories and the third one is the one to know about**: a border whose colour changes with state is a state marker, not a divider, and is outside the seam rule at whatever weight it carries. It exists because the rule as first written would have thinned `.validation-panel` to 1px, and that border **is** the verdict: red on a reject, amber on accept with warnings, green on accept. Three markers are pinned by test, on width and on the fact that the colour varies. `.bundle-warning` is the honest exception and is held by name, because its colour does not vary. **`.workspace-panel` was named as a card in ruling 2 and was left flat**, because it has no bounded surface and making it one means choosing a fill that pass 2 does not authorise; both candidate fills are wrong for a stated reason. That needs one line from the product owner. Evidence in `docs/evidence/theme-pass-2-evidence.md`. **Verified in Production on 16 September 2026**, see the top entry. `.workspace-panel` was settled by ruling 10: it stays flat, and section 9a now defines a card as a bounded surface.
- **Assay theme pass 1 is built and merged, 16 September 2026.** Pull request 103, merge commit `9fb0a638038c9862f4dbeb9fc2ae1f2531ba9b29`. The suite is 273 tests and passes on `main` at that commit. Pass 1 is the quietening half of the theme and only takes weight away: the row rule and row hover on the four workspace grids, a 1px `--control-border` on inputs in place of the 2px ink, and an `--accent-pressed` hover on action buttons. No radius, no shadow, no fill, no type change. Rendered text on eleven screens is identical to the unmodified app, so nothing a user reads changed. **One value was not built as specified and is now approved:** the changeset's `--row-rule` `#eeebe9` was chosen against a white card and measures 1.02:1 against Assay's `--surface`, on the lighter side of it, so the row divisions disappeared. `#dcd9d7` re-derives the changeset's own relationship against Assay's ground. The product owner approved it and made the method a rule, recorded in section 9a as "re-derive, do not copy". **Verified in Production on 16 September 2026**, see the top entry.
- **Pass 2 is specified and not built.** It is held on two answers, recorded as Q13 and Q14 in `docs/assay-theme-open-questions.md`. Q13 is `--slate-tint`, re-derived to `#d4d6d9` for the table header fill with `#c8cacd` for the sort hover, previewed and reverted rather than applied; it also records that `--slate-tint` has three jobs on three different grounds in the changeset and cannot be one token in Assay. Q14 is the four heavy seams, and the finding in it is that `.validation-panel`'s 5px border **is** the verdict, red on a reject, amber on accept with warnings, green on accept, so dropping it to 1px weakens the colour half of a state that section 9 requires to be encoded twice. Ruling 3 was corrected by the product owner after the seam count: the marker follows the job, not the colour, so `form` and `.resolution-form` keep their heavier marker and `.analysis-panel`, `.quality-hero` and `.routing-headline` drop to 1px.
- **Band 2.9 is built and merged, 16 September 2026.** Pull request 101, merge commit `692a0b91e2b870766df4d9586950f44979828bcf`. The suite is 268 tests and passes on `main` at that commit. Three stories in `forecast-app/static/index.html`: the two `:root` blocks became one, `--muted` moved to the marketing site's `#66615f`, and every control now declares its own colour and its own radius. The last is the iOS control colour defect recorded below under 6 September as "far worse in Assay", and it was: 252 of 397 visible control readings computed the platform default `rgb(0, 0, 0)` against an app ink of `rgb(63, 61, 59)`, and after the change 0 of 397. **The product owner checked the buttons on a phone before the merge and reported them correct.** Which device and which screens are not recorded, because he did not state them. No fixture, expectation, golden file or schema was touched. The rendered text of eleven screens is identical before and after, so nothing a user reads changed. Evidence in `docs/evidence/2.9-evidence.md`. **Verified in Production on 16 September 2026**, see the top entry.
- **`CLAUDE.md` section 9 was amended by the product owner on 16 September 2026, and Assay is now a documented exception to the zero radius rule.** Section 9a carries it. The order is the part that matters and is recorded in the pull request: the build session reported that the approved theme changeset could not be applied without breaking section 9 and recommended against applying it; the product owner amended the rule; only then were the tests changed. The two assertions on the literal `*{border-radius:0!important}` became `test_no_radius_exceeds_the_assay_bound`, which holds the 4px ceiling rather than one pinned value, and the universal reset was removed from the app and the gate screen because `!important` on the universal selector made the amended rule unimplementable. Removing it moved nothing: 1174 control readings across ten screens compute identically with and without it, and every radius on the page is still `0px`. Two later rulings the same day closed Q8 and Q9. **The theme itself is not built.** The rule permits it; nothing applies 4px to a card, a shadow to a panel, or the slate palette to anything.
...
**The theme is finished, verified in Production on 16 September 2026, and nothing on it is waiting on anyone.** Band 2.9, theme pass 1, theme pass 2, rulings 1 to 18, all nineteen questions and the phone check are done. **All nineteen questions raised against the theme are closed**: thirteen answered or corrected by a ruling, three moot, and two fixed in the last pull request. Every heading in `docs/assay-theme-open-questions.md` carries its own status.
...
`docs/production-smoke-test.md` holds the checklist and the 20 September result. **It stays runnable**, with a blank results block above the recorded runs, so the next release is checked the same way rather than from memory.

**Two things are unchecked rather than failed, and they are the honest edges of that pass.** The validation bar was only ever seen amber, because the app's own downloadable sample produces a warning; red and green need a file the app does not serve. The quality exception bar was not reached for the same reason. Neither is a defect and neither blocks anything, but a future band that wants them covered has to give a phone a way to download a rejected file and a not usable one.

**The next build is whatever the planning session briefs.** Story 2.6, the staleness gap, and sprint 3 forecasting methods remain unblocked; `docs/2.2-open-questions.md` Q10 describes the staleness story. Band 2.10 still awaits its third planner test, which is unrelated to the theme and is described below.

Superseded on 16 September 2026 when the phone check came back, kept for the record: **the Production check was the only thing outstanding and could not be run from a build session.** It was run by the product owner and all nine checks passed.

Superseded, kept for the record: **Theme pass 2 is built and awaiting acceptance in pull request 105. After it merges, four questions are open and none blocks anything**, all in `docs/assay-theme-open-questions.md`: Q15, buttons and inputs are permitted 4px by section 9a and pass 2 did not scope them, so thirteen softened cards currently hold hard-cornered controls; Q16, the table header underline is a second separator now the header has a fill; Q17, one summary figure is a date range rather than a numeral and is now the widest thing on its panel at 36px; and **Q18, which is the one worth acting on**: the page scrolls sideways at narrow widths. **Q18 was first raised with the wrong cause and is corrected in place**; read the correction rather than the original. It is not the grid, which already sits in a `.table-wrap` and scrolls itself. It is the run footer, which prints a 64 character manifest hash with no break opportunity, so at narrow widths it cannot wrap and pushes the document. Pre-existing, not the theme's. The fix is one declaration, `word-break:break-word` on `.report-footer`, the pattern `.provenance-grid strong` already uses on the same kind of value: 90px of overflow without it, 0px with it, measured. Not applied, because the product owner ruled a band on the original diagnosis and the shape of the work is what changed.

Superseded, kept because it was the starting point for half a day: **pass 2 was held on Q13 and Q14.** Both were answered on 16 September 2026 by rulings 6 to 9. Both are derivations and findings put to the product owner, not work waiting on a builder. Pass 2 is then: `--slate-tint` at the approved value, the seams under the corrected ruling 3, the radius and one card elevation under section 9a, and the type scale at 36px/800 figures and 12px/700 labels. **A card is a bounded panel holding one kind of output that could be moved elsewhere on the page without breaking what is around it**, which is the eight workspace panels, the quality hero, the validation panel, the provenance block and the bundle view; full bleed sections that divide the page are not cards and stay flat. An ambiguous panel is left flat and listed rather than guessed towards more radius.

Superseded, kept because it was the starting point for half a day: **the theme could not start until Q3 was answered.** Q3 was answered on 16 September 2026 with three rulings and the two pass split. The changeset "Assay app theme and dashboard shell" is approved and section 9a now permits it, but the changeset describes a dashboard Assay does not have: a 232px sidebar with two nav groups, three named metric cards, an actual-against-forecast chart and a five column product table. Assay is a masthead, a single file form, a portfolio form and an eight tab workspace. Under the product owner's correction of 16 September, "styling only, same screens, same words", every one of those structures is out of scope, and what survives has to be restated as rules against the screens that exist. That restatement is the planning session's work and has not been done. Do not start by guessing what the mock means here.

Four smaller questions are open in the same file: Q4 on whether the theme adds nav items, Q5 on the white button label that measures 3.18:1 and fails, Q6 on twelve raw hex literals of the abandoned palette still drawing two charts on the single file diagnostic screen, and Q7 on five tokens declared and never used. Q5 and Q6 both need settling before or during the theme build, because the theme touches the same code.
