...
- Story 1.4 is implemented locally on `codex/story-1-4-run-bundles`. Automated checks pass, but Preview, manual acceptance, pull request, merge and Production checks have not yet occurred.
...
1. Review the branch diff and commit the implementation.
2. Push the branch and inspect the Forecast Diagnostic Vercel Preview.
3. Manually export, alter, refuse, reopen and reproduce the fixed quality fixture.
4. Decide whether forecast rerun must be exposed in this story or remains a tested contract capability for a later UI increment.
5. Open and merge a pull request only after Preview acceptance, then run the Production smoke test.
