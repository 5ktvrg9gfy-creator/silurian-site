...
- Story 2.3 planner action view merged in pull request 50 at `655b984` on 2 September 2026. A fresh clone of `main` at that commit has a clean working tree and passes 119 tests. Fixtures 30 and 31 still produce their recorded results. The run manifest schema is 1.6 and the confidential bundle schema is 1.3. The Vercel Preview deployments for the merged head were Ready with no failed check. The Production smoke test and the fixture 31 routing and planner action checks against Production have not been run from the build session, whose egress policy blocks `vercel.app`; the owner runs them and records the result here.
- Sprint 2 is not closed. Every code criterion of Stories 2.1, 2.2 and 2.3 passes. Criterion 16 of Story 2.3, the one minute action test on a person who did not build the tool, is outstanding and is the only thing between the current position and a closed sprint.
...
Sprint 2 is not closed. Stories 2.1, 2.2 and 2.3 are merged, `main` is at `655b984`, and every code criterion passes. Criterion 16 of Story 2.3, the one minute action test in section 7 of `docs/briefs/2.3-build-brief.md`, is outstanding: one person who did not build the tool, the screen alone, no narration, asked what they would do first and about which line. It passes if they name PKG-50301 or RTG-60301 and their answer is a conversation with the customer about order patterns, not a better forecast or a different model. It is the only thing between the current position and a closed sprint, and its result belongs in this file. Also still to record here: the Production smoke test and the fixture 31 routing and planner action checks against Production for Stories 2.2 and 2.3. Story 2.6, the staleness gap, and sprint 3 forecasting methods start only after sprint 2 closes. Then read `docs/2.2-open-questions.md`: Q1 and Q2 are closed by `expected_routing.json` v1.1, and Q10 names the staleness story that measures discontinuation on the last period with demand, recommended as a separate small story after 2.2 rather than folded into 2.3. Story 2.3, the routing-aware planner action view, starts from current `main` once 2.2 is merged: it fills the drawer's empty do-this slot, captures a resolution code and optional note through the interface, records the new pass in the manifest options, and shows the open items list. Any forecasting method or baseline remains sprint 3.
