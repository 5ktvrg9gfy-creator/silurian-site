...
- Marketing preview branch `codex/preview-linkedin-badge` replaces the rectangular email and LinkedIn text buttons with matching white circular badges containing orange envelope and `in` symbols, reversing to the logo's lighter charcoal with white symbols on rollover. The email address remains present in the `mailto:` link and therefore remains machine-readable. Production remains unchanged unless the preview is approved and merged.
