...
- Story 1.4 is implemented on `codex/story-1-4-run-bundles` in pull request 29. Quality and forecast-bundle Preview acceptance checks passed. Merge and Production checks have not yet occurred.
...
1. Wait for the refreshed pull-request checks after the handoff-only commit.
2. Merge pull request 29 when all checks pass.
3. Wait for the Production deployment and repeat export, reopen and reproduce with the fixed quality fixture.
4. Confirm Production carries the measured determinism evidence expected from Story 1.3.
5. Record the merge reference and Production result here.
