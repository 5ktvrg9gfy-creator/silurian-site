...
- **CSS authority.** The homepage is governed by the `<style>` blocks and tokens embedded in the root `index.html`. `styles.css` and `ds-styles.css` sit in the repository root but the homepage does not link them. Editing a similarly named standalone stylesheet will appear to do nothing. Read the embedded styles first.
  ...
  The same trap runs inside `index.html` itself. It holds three `<style>` blocks and defines some tokens twice, the later definition winning. `--color-divider` is `color-mix(in srgb, #3f3d3b 40%, transparent)` at line 28 and plain `#3f3d3b` at line 313, so every rule on the site is drawn solid, not at 40 percent. The second block also retunes `--color-accent` to `#ec6917` and adds `--color-accent-600: #d1601a` and `--color-accent-700: #8f3f0c`. Read to the end of the file before trusting a token value, and confirm the rendered pixel.
